...
public class VA_MeshTree
{
	...
	public Vector3 FindClosestPoint(Vector3 point)
	{
		if (Nodes.Count > 0)
		{
			...
			for (var i = searchResults.Count - 1; i >= 0; i--)
			{
				...
				var triangleDistance = (trianglePoint - point).sqrMagnitude;
	...
	private void Search(Node node)
	{
		...
		var nodeNear = MinimumDistance(node);
		...
		var nodeFar = MaximumDistance(node);
	...
	private float MinimumDistance(Node node)
	{
		return node.Bound.SqrDistance(searchPoint);
	}

	// Compute the sqrDistance to each node corner and return the highest
	private float MaximumDistance(Node node)
	{
